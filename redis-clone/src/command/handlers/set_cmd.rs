...
pub fn handle_sadd(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() < 2 {
    ...
    let set = store.get_or_insert_set(key);
    let added = args[1..].iter().filter(|m| set.insert((*m).clone())).count();
    RespValue::Integer(added as i64)
}

pub fn handle_srem(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() < 2 {
...
pub fn handle_smembers(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() != 1 {
...
pub fn handle_sismember(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() != 2 {
...
pub fn handle_scard(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() != 1 {
...
mod tests {
    ...
    fn sadd_and_scard() {
        ...
        let r = handle_sadd(&[b"s".to_vec(), b"a".to_vec(), b"b".to_vec(), b"c".to_vec()], &mut store);
        assert_eq!(r, RespValue::Integer(3));
        assert_eq!(handle_scard(&[b"s".to_vec()], &mut store), RespValue::Integer(3));
    }

    #[test]
    fn sadd_no_duplicates() {
        let mut store = Store::new();
        handle_sadd(&[b"s".to_vec(), b"a".to_vec()], &mut store);
        let r = handle_sadd(&[b"s".to_vec(), b"a".to_vec(), b"b".to_vec()], &mut store);
    ...
    fn srem() {
        let mut store = Store::new();
        handle_sadd(&[b"s".to_vec(), b"a".to_vec(), b"b".to_vec()], &mut store);
        assert_eq!(handle_srem(&[b"s".to_vec(), b"a".to_vec()], &mut store), RespValue::Integer(1));
        assert_eq!(handle_srem(&[b"s".to_vec(), b"a".to_vec()], &mut store), RespValue::Integer(0));
    }
    ...
    fn sismember() {
        let mut store = Store::new();
        handle_sadd(&[b"s".to_vec(), b"a".to_vec()], &mut store);
        assert_eq!(handle_sismember(&[b"s".to_vec(), b"a".to_vec()], &mut store), RespValue::Integer(1));
        assert_eq!(handle_sismember(&[b"s".to_vec(), b"b".to_vec()], &mut store), RespValue::Integer(0));
    }
    ...
    fn wrongtype() {
        ...
        assert!(matches!(
            handle_sadd(&[b"k".to_vec(), b"x".to_vec()], &mut store),
