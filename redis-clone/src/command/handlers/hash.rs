...
pub fn handle_hset(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    ...
    let hash = store.get_or_insert_hash(key);
    let mut added = 0i64;
...
pub fn handle_hget(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() != 2 {
...
pub fn handle_hdel(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() < 2 {
...
pub fn handle_hgetall(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() != 1 {
...
pub fn handle_hexists(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() != 2 {
...
pub fn handle_hlen(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() != 1 {
...
pub fn handle_hkeys(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() != 1 {
...
pub fn handle_hvals(args: &[Vec<u8>], store: &mut Store) -> RespValue {
    if args.len() != 1 {
...
mod tests {
    ...
    fn hset_hget_round_trip() {
        ...
        let r = handle_hset(&[b"h".to_vec(), b"f1".to_vec(), b"v1".to_vec(), b"f2".to_vec(), b"v2".to_vec()], &mut store);
        assert_eq!(r, RespValue::Integer(2)); // 2 new fields

        assert_eq!(handle_hget(&[b"h".to_vec(), b"f1".to_vec()], &mut store),
            RespValue::BulkString(Some(b"v1".to_vec())));
        assert_eq!(handle_hget(&[b"h".to_vec(), b"missing".to_vec()], &mut store),
    ...
    fn hset_update_does_not_increment() {
        let mut store = Store::new();
        handle_hset(&[b"h".to_vec(), b"f".to_vec(), b"v1".to_vec()], &mut store);
        let r = handle_hset(&[b"h".to_vec(), b"f".to_vec(), b"v2".to_vec()], &mut store);
    ...
    fn hdel_and_hexists() {
        let mut store = Store::new();
        handle_hset(&[b"h".to_vec(), b"f".to_vec(), b"v".to_vec()], &mut store);
        assert_eq!(handle_hexists(&[b"h".to_vec(), b"f".to_vec()], &mut store), RespValue::Integer(1));
        handle_hdel(&[b"h".to_vec(), b"f".to_vec()], &mut store);
        assert_eq!(handle_hexists(&[b"h".to_vec(), b"f".to_vec()], &mut store), RespValue::Integer(0));
    }

    #[test]
    fn hlen() {
        let mut store = Store::new();
        handle_hset(&[b"h".to_vec(), b"a".to_vec(), b"1".to_vec(), b"b".to_vec(), b"2".to_vec()], &mut store);
        assert_eq!(handle_hlen(&[b"h".to_vec()], &mut store), RespValue::Integer(2));
    }
    ...
    fn wrongtype() {
        ...
        assert!(matches!(handle_hset(&[b"k".to_vec(), b"f".to_vec(), b"v".to_vec()], &mut store),
